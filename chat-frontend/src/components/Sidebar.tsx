...
import { ConversationSummary, StatusEnum } from '@/types/chat';
  ...
  useEffect(() => {
    const refresh = () =>
      getHistory()
        ...
        .catch(() => {});
    refresh();
    window.addEventListener('chat-ended', refresh);
    return () => window.removeEventListener('chat-ended', refresh);
  }, []);
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto p-2">
        ...
        {history.map((item) => {
          const isEnded = item.status === StatusEnum.Ended;
          return (
            <button
              key={item.conversationId}
              title={`${item.title ?? 'Untitled'} (${isEnded ? 'Ended' : 'In progress'})`}
      ...
      <div className="border-t border-zinc-200 p-3 dark:border-zinc-800">
        {isOpen && (
          <div className="flex items-center justify-center gap-1 text-xs text-zinc-400 dark:text-zinc-600">
            <ShieldAlert size={12} />
            Apate AI
          </div>
        )}
